...
class ExceptionControllerAdvice {
	...
	public ResponseEntity<JSONObject> handleConstraintMissing(ConstraintViolationException ex) {
		JSONObject response = new JSONObject();
		JSONArray details = new JSONArray();
		...
		ex.getConstraintViolations().forEach((error) -> {
			JSONObject detail = new JSONObject();
			try {
				detail.put(error.getPropertyPath(), error.getMessage());
				details.add(detail);
			} catch (Exception e) {
				logger.error(e);
			}
